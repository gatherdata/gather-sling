 ...
 * 
 */
public class OrbeonCrudServlet extends SlingAllMethodsServlet implements OptingServlet {

    private static final long serialVersionUID = 1837674982291697074L;

    private final Log log = LogFactory.getLog(OrbeonCrudServlet.class);
    ...
    protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response) throws ServletException,
        ...
        if (resource != null) {
            OrbeonFormResource orbResource = request.getResource().adaptTo(OrbeonFormResource.class);
            if (orbResource.exists()) {
                ServletOutputStream os = response.getOutputStream();

                os.print((String) orbResource.getForm().getFormTemplate());
            } else {
    ...
    protected void doPut(SlingHttpServletRequest request, SlingHttpServletResponse response) throws ServletException,
        ...
        OrbeonFormResource orbResource = request.getResource().adaptTo(OrbeonFormResource.class);
        MutableFormTemplate putTemplate = new MutableFormTemplate();
        URI resourceUid = orbResource.getUid();
        
        DateTime now = new DateTime();
        
        if (!orbResource.exists()) {
            putTemplate.setUid(resourceUid);
            putTemplate.setDateCreated(now);
            putTemplate.setUid(orbResource.getUid());
            putTemplate.setFormStyle(FormTemplateStyle.ORBEON.toString());
            putTemplate.setFormType("text/xhtml");
        }
        putTemplate.setDateModified(now);

        // read the form template body
        ...
        putTemplate.setFormTemplate(content.toString());

        formCatalog.save(putTemplate);
    ...
    public boolean accepts(SlingHttpServletRequest request) {
        ...
        if (request.getRequestPathInfo().getResourcePath().startsWith(OrbeonResourceProvider.PROVIDER_ROOT)) {
            return true;
        }
        return false;
    }
